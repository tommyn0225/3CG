...
function UI:mousereleased(x, y, button)
    ...
    if x >= dx and x <= dx+pileW and y >= dy and y <= dy+pileH then
        if self.dragging.fromHand and not UI.discardedThisTurn then
    ...
    if y >= hy and y <= hy + CARD_H then
        if self.dragging.fromHand then
            ...
            newIndex = math.max(1, math.min(newIndex, #p.hand + 1))
            table.insert(p.hand, newIndex, c)
        else
            -- Return card to hand
            p:returnCardToHand(c, Game.board)
        end
        self.dragging = nil
        return
    end
    ...
    for loc=1,3 do
        ...
        if Game.state == "scoring" then
            ...
            if powerDiff ~= 0 then
                ...
                lg.printf(diffText, x + (laneW - fw)/2, laneY + laneH/2 - fontSize/2, fw, 'center')
                lg.setFont(oldFont)
            end
        end
    end
